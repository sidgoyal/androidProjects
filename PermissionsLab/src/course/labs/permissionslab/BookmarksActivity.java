...
public class BookmarksActivity extends Activity {
	...
	private void loadBookmarks() {
		...
		String text = "";
		System.out.println("Before cursor  query");
		Cursor query = getContentResolver().query(Browser.BOOKMARKS_URI,
				projection, null, null, null);
		System.out.println("after cursor query");
		
		System.out.println("before moveToFirst");
		query.moveToFirst();
		System.out.println("after moveToFirst");
		System.out.println("before while");
		
		while (query.moveToNext()) {
			System.out.println("inside while before TITLE");
			text += query.getString(query
					.getColumnIndex(Browser.BookmarkColumns.TITLE));
			text += "\n";
			System.out.println("after title, before url");
			text += query.getString(query
