...
public class CreateStoryActivity extends StoryActivityBase {
	...
	private void launchSoundIntent() {
        ...
        // TODO - Use getOutputMediaFile() to create a new  filename for this specific sound file
        File soundFile = getOutputMediaFile(MEDIA_TYPE_AUDIO);
 
        // TODO - Add the filename to the Intent as an extra. Use the Intent-extra name from the SoundRecordActivity class, EXTRA_OUTPUT
        soundIntent.putExtra(SoundRecordActivity.EXTRA_OUTPUT, soundFile.getAbsolutePath());
 
        // TODO - Start a new activity for result, using the new intent and the request code MIC_SOUND_REQUEST
	...
	private void launchCameraIntent() {
		...
		// TODO - Create a new intent to launch the MediaStore, Image capture function Hint: use standard Intent from MediaStore class
        // See: http://developer.android.com/reference/android/provider/MediaStore.html
        Intent cameraIntent = new Intent(MediaStore.ACTION_IMAGE_CAPTURE);
 
        // TODO - Set the imagePath for this image file using the pre-made function getOutputMediaFile to create a new filename for this specific image;
        Uri cameraFile = getOutputMediaFileUri(MEDIA_TYPE_IMAGE);
        fragment.imagePath = cameraFile;
 
        // TODO - Add the filename to the Intent as an extra. Use the Intent-extra name from the MediaStore class, EXTRA_OUTPUT
        cameraIntent.putExtra(MediaStore.EXTRA_OUTPUT, cameraFile);
 
        // TODO - Start a new activity for result, using the new intent and the request code CAMERA_PIC_REQUEST
	...
	private void launchVideoCameraIntent() {
        ...
        // TODO - Set the fileUri for this video file using the pre-made function getOutputMediaFile to create a new filename for this specific video;
        Uri videoFile = getOutputMediaFileUri(MEDIA_TYPE_VIDEO);
 
        // TODO - Add the filename to the Intent as an extra. Use the Intent-extra name from the MediaStore class, EXTRA_OUTPUT
        videoIntent.putExtra(MediaStore.EXTRA_OUTPUT, videoFile);
 
        // TODO - Specify as an extra that the video quality should be HIGH. Use the Intent-extra name, EXTRA_VIDEO_QUALITY, from the MediaStore class
